...
const aiAgent = async (prompt, sysPrompt, useTool) => {
    try {
        history.push({
            role: "user",
            content: prompt,
        ...
        \n
         ------------------------------------------------------------------------------------------------------- \n`);
            ...
            if (!tollCalls || res.choices[0]?.message?.tool_calls?.length === 0) {
                console.log("Final Res.choices[0]: ", res.choices[0].message.content, "\n Total token used: ", tokenCount);
                break;
            }
...
const main = async () => {
    rl.question("Ask anything about your system and backend (type 'exit' to quite): ", async (answer) => {
